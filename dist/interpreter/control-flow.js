...
export async function executeFor(ctx, node) {
        ...
        for (const value of words) {
            ...
            try {
                for (const stmt of node.body) {
                    ...
                    ({ stdout, stderr } = appendExecResultBytes({ stdout, stderr }, stmtResult));
                ...
                if (loopResult.action === "error") {
                    ...
                    const bodyResult = { ...result(stdout, stderr, loopResult.exitCode ?? 1), stdoutKind: "bytes" };
    ...
    const bodyResult = { ...result(stdout, stderr, exitCode), stdoutKind: "bytes" };
    return applyRedirections(ctx, bodyResult, node.redirections);
}
