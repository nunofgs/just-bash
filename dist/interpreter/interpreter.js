 ...
 */
import { appendExecResultBytes, bytesFromHostText, latin1FromBytes, readBytesFrom, } from "../encoding.js";
    ...
    async executeSimpleCommandInner(node, stdin) {
        ...
        for (const redir of node.redirections) {
            ...
            if (redir.operator === "<<<" && redir.target.type === "Word") {
                ...
                stdin = bytesFromHostText(`${await expandWord(this.ctx, redir.target)}\n`);
                continue;
            }
            if (redir.operator === "<" && redir.target.type === "Word") {
                try {
