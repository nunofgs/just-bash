 ...
 */
function pipelineWriteEncoding(content) {
...
export async function applyRedirections(ctx, result, redirections, preExpandedTargets) {
    ...
    const writeEncoding = (kind, encoding, content) => {
        if (kind === "bytes")
            return "binary";
        if (kind === "text")
            return "utf8";
    ...
    const stdoutEnc = (content) => writeEncoding(stdoutKind, stdoutEncoding, content);
    // Stderr's shape is signalled by `stderrKind`; the legacy
    // `stdoutEncoding: "binary"` flag never applied to stderr.
    const stderrEnc = (content) => writeEncoding(stderrKind, undefined, content);
    ...
    const toByteString = (content, kind, encoding) => latin1FromBytes(stdoutAsBytes({
        stdout: content,
        stdoutKind: kind,
                ...
                // Handle writing to a user-allocated FD (>&$fd)
                    ...
                    if (!Number.isNaN(targetFd)) {
                        ...
                        if (fdInfo?.startsWith("__file__:")) {
                            ...
                            if (fd === 1) {
                                await ctx.fs.appendFile(resolvedPath, stdout, stdoutEnc(stdout));
                                stdout = "";
                            }
                            else if (fd === 2) {
                                await ctx.fs.appendFile(resolvedPath, stderr, stderrEnc(stderr));
                                stderr = "";
                            }
                        }
                            ...
                            if (parsed) {
                                if (fd === 1) {
                                    await ctx.fs.appendFile(parsed.path, stdout, stdoutEnc(stdout));
                                    stdout = "";
                                }
                                else if (fd === 2) {
                                    await ctx.fs.appendFile(parsed.path, stderr, stderrEnc(stderr));
                                    stderr = "";
                                }
                            }
                        }
