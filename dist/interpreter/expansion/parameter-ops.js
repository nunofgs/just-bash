 ...
 */
import { decodeBytesToUtf8, encodeUtf8ToBytes, latin1FromBytes, unsafeBytesFromLatin1, } from "../../encoding.js";
...
export async function handlePatternRemoval(ctx, value, operation, expandWordPartsAsync, expandPart) {
    ...
    if (operation.pattern) {
        for (const part of operation.pattern.parts) {
            if (part.type === "Glob") {
                ...
                regexStr += escapeRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(expanded)));
            }
            else if (part.type === "ParameterExpansion") {
                const expanded = await expandPart(ctx, part);
                regexStr += patternToRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(expanded)), operation.greedy, extglob);
            }
            else {
                const expanded = await expandPart(ctx, part);
                regexStr += escapeRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(expanded)));
    ...
    const reencode = (s) => latin1FromBytes(encodeUtf8ToBytes(s));
    // 'su' flags: dotall + Unicode (so `.` matches one codepoint).
    if (operation.side === "prefix") {
        return reencode(createUserRegex(`^${regexStr}`, "su").replace(decodedValue, ""));
    }
...
export async function handlePatternReplacement(ctx, value, operation, expandWordPartsAsync, expandPart) {
    ...
    if (operation.pattern) {
        for (const part of operation.pattern.parts) {
            if (part.type === "Glob") {
                regex += patternToRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(part.pattern)), true, extglob);
            }
            else if (part.type === "Literal") {
                regex += patternToRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(part.value)), true, extglob);
            }
            else if (part.type === "SingleQuoted" || part.type === "Escaped") {
                regex += escapeRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(part.value)));
            }
            else if (part.type === "DoubleQuoted") {
                const expanded = await expandWordPartsAsync(ctx, part.parts);
                regex += escapeRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(expanded)));
            }
            else if (part.type === "ParameterExpansion") {
                const expanded = await expandPart(ctx, part);
                regex += patternToRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(expanded)), true, extglob);
            }
            else {
                const expanded = await expandPart(ctx, part);
                regex += escapeRegex(decodeBytesToUtf8(unsafeBytesFromLatin1(expanded)));
    ...
    try {
        ...
        if (operation.all) {
            ...
            let match = re.exec(decodedValue);
            while (match !== null) {
...
export function handleLength(ctx, parameter, value) {
    ...
    if (/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(parameter) && isArray(ctx, parameter)) {
        ...
        return String([...decodeBytesToUtf8(unsafeBytesFromLatin1(firstElement))].length);
    }
