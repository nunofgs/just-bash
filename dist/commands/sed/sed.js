...
export const sedCommand = {
    ...
    async execute(args, ctx) {
        ...
        const decodedScripts = scripts.map((s) => decodeBytesToUtf8(unsafeBytesFromLatin1(s)));
        const { commands, error, silentMode } = parseMultipleScripts(decodedScripts, extendedRegex);
        if (error) {
            return {
                stdout: "",
                stderr: `sed: ${error}\n`,
